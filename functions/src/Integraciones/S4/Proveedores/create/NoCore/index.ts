...
import * as schema from "./ProveedorCreateNC_schema.json";
...
export const createProveedorNCS4 = functions
    ...
    .https.onCall(
        async (data, context) => {
            ...
            const result = await VerifyIntegrationRequest(
                data, context,
                "Integraciones-S4-Proveedores-CreateNC",
                [
                    "service",
                    "tester",
                    "cpp",
    ...
    try {
        ...
        if (resp.data.message.returnMessage.includes("creado")) {
            ...
            if (providerDoc) {
                ...
                // Recorrer la organization y agregar el SAP_idProvider

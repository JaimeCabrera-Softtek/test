...
import { body01 } from "./helpers/segment_body_helpers/1";

/**
 * Enviar el segmento 1 a SAP
 * 
 * @param prodRef Referencia al nodo del producto en /AltaMateriales
 */
export const segmento01 = async (prodRef: Reference): Promise<CloudRes> => {
    ...
    const _temp = await body01(product_data);

    if (process.env.CPI_SEGMENT_ENV === 'PROD') {
        console.log('Enviando a CPI JobID', product_data.push_id, "Segmento 1");
        //#region envío
        try {
            const url: string = process.env.ALTAMATERIALES_CPI_R52_SEGMENT01 ?? '';
            // console.log('CPI Segment 01', 'req.url', url);
            ...
            console.log('CPI Segment 01', 'req.body', JSON.stringify(_temp));
            console.log('CPI Segment 01', 'res.data', JSON.stringify(res.data));
            ...
            console.log("JobID", product_data.push_id, "Segmento 1 enviado a CPI")

            return res_ok;
        } catch (e) {
            console.log("Error enviando JobID", product_data.push_id, "Segmento 1")
            throw e;
        }
    } else {
